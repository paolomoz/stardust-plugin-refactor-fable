...
function normalizeUrl(u, base) {
  ...
  url.hash = '';
  // stardust.style 404s on slash-stripped paths (/docs -> 404, /docs/ -> 200)
  // with no redirect between variants — keep the sitemap's trailing slashes verbatim.
  return url.href;
}
...
async function capturePage(context, url, slug, args) {
  ...
  // settle after return-to-top so entry animations (hero reveal) reach their
  // final state before the visibility filter reads computed opacity.
