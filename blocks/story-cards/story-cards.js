export default function decorate(block){
  [...block.children].forEach((row,i)=>{row.classList.add('story');if(i===0)row.classList.add('lead');
    const [media,body]=row.children;
    if(media)media.classList.add('ph');
    if(body){body.classList.add('sbody');const p=body.querySelector('p');if(p)p.classList.add('date');}
