/**
 * why-variation — ledger 2-up (data-section "why-structured-variation").
 *
 * Section head (deck + h2) is authored as DEFAULT CONTENT before the block;
 * the block reabsorbs it (ENCODE contract). Block rows:
 *   body paragraph (why-body)
 *   one row per ledger item: <h3> + paragraph
 *
 * Decode: #62 flatten-first collector; #52 h3-boundary segmentation;
 * #53 matches-or-descendant; in-table head kept as back-compat fallback.
 */
...
/* section head reabsorption — sibling of the SECTION-level wrapper, both runtimes */
...
export default async function decorate(block) {
  ...
  /* back-compat: head authored in-table (leading rows up to the h2) */
  ...
  preNodes.forEach((node) => {
    if (!node.textContent.trim()) return;
    node.classList.add('why-body');
    wrap.append(node);
  });

  const ledger = document.createElement('div');
  ledger.className = 'ledger-2up';
  units.forEach((unit) => {
    const item = document.createElement('div');
    const h3 = document.createElement('h3');
    const src = unit.title.querySelector('h3, h4') || unit.title;
    h3.append(...src.childNodes);
    item.append(h3, ...unit.body);
    ledger.append(item);
  });
  if (units.length) wrap.append(ledger);
